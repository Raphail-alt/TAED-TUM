...
// Demo-mode auth used when Supabase keys are not configured. Data lives in this browser only.
...
export default function LocalAuthProvider({ children }) {
  ...
  const signUp = useCallback(
    async ({ role, name, email, password }) => {
      ...
      setAccounts(next)
      startSession({ id: account.id, role, name: account.name, email: account.email })
  ...
  const logIn = useCallback(
    async (email, password) => {
      ...
      startSession({ id: account.id, role: account.role, name: account.name, email: account.email })
    },
    [startSession],
  )
  ...
  const reloadAccounts = useCallback(() => setAccounts(read(ACCOUNTS_KEY, [])), [])
  ...
  const value = useMemo(
    ...
    [user, accounts, reloadAccounts, signUp, logIn, logOut],
  )
